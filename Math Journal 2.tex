...
\exTwo\ul{Theorem:} \hypertarget{Noetherian Rings allow prime factorization of ideals}{Let} $A$ be a nontrivial Noetherian ring.
\begin{enumerate}
	...
	\begin{myIndent}\exThreeP
		...
		\begin{myIndent}\myComment
			Remark 1: Given any proper ideal $J \lhd A$ we can apply this result to the ring $A/J$ to write $J$ as a product of prime ideals.\newpage
...
A ring $A$ is \udefine{Jacobson} is every prime ideal $P \lhd A$ is an intersection of maximal ideals.\newpage
...
\[\begin{tikzcd}
	{\mathcal{P}(\mathbb{A}^n)} && {\mathcalli{P}(B)}
...
\begin{myIndent}\pracTwo
	Why?\\
	We can write $F = V(J) \cap \Max(k[x, y])$ for some $J \lhd k[x, y]$. But now since $k[x, y]$ is a Noetherian ring, we can find prime ideals $Q_1, \ldots, Q_n$ (not all necessarily distinct) such that $J = Q_1 \cdots Q_n$.
	\begin{myIndent}\color{VioletRed}
		Proof:\\
		$k[x, y]$ being Noetherian implies that $k[x,y]/J$ is as well. Thus, we can apply\\ the theorem on \inLinkRap{Noetherian Rings allow prime factorization of ideals}{page 900} followed by the ideal correspondance theorem to get\\ the factorization we want.\retTwo
	\end{myIndent}

	But now $V(J) = \bigcup_{i=1}^n V(Q_j)$. Also since $F \neq \mathbb{A}^2 \cong \Max(k[x, y])$, we know that $J \neq \{0\}$. In turn, we can't have that $Q_i = \{0\}$ for any $i$. Hence, we now just apply the prior proposition in order to describe what $V(Q_i) \cap \Max(k[x, y])$ looks like for each $i$.\retTwo
\end{myIndent}
...
{\centering$I(Z(J)) = \sqrt{J}$ ($ = \bigcap_{P \in V(J)} P = \{f \in B : f^n \in J \text{ for some } n \in \mathbb{Z}_{\geq 0}\}$).\retTwo\par}
...
\begin{myIndent}\pracTwo
	After all, if $a_1 - a_2 \in \Ann_A(M)$ then $(a_1  - a_2)m  = 0$. Hence $a_1 m = a_2 m $.\newpage
\end{myIndent}

\begin{myIndent}\myComment
	Note: If $M$ is finitely generated then $\dim(M)$ is equal to the maximal chain length of\\ $\supp(M) \subseteq \Spec(A)$. (See \inLinkRap{placeholder}{pages 672-673}).\retTwo
\end{myIndent}
...
\begin{myIndent}\pracTwo
	...
	By the theorem on \inLinkRap{Noetherian Rings allow prime factorization of ideals}{pages 899-900}, we can find a bunch of primes $P_1,  \ldots,  P_n$ with\\ $0 = P_1 \cdots P_n$. Without loss of generality, we can assume $n$ is minimal (and since $A$ is not an integral domain, we know $n \geq 2$).\retTwo
...
\begin{myIndent}\exThreeP
	...
	$(\Longleftarrow)$\\
	Suppose $a \notin J(A)$. Then there exists $M \in \Max(A)$ such that $a \notin M$. Yet then since $\langle a, M\rangle$ is an ideal properly containing the maximal ideal $M$, we must have that $A = \langle a, M\rangle$. Hence there exists $b \in A$ and $m \in M$ with $ab + m = 1$. Or in other words, $1 - ab = m \in M$. $\blacksquare$\retTwo
\end{myIndent}
...
\begin{enumerate}
	...
	\begin{myIndent}\HexOne
		...
		But now since $Z_j \subseteq Z = \bigcup_{i=1}^n F_i$, we know that $Z_j \cap F_1$ and $Z_j \cap (\bigcup_{i=2}^m F_i)$ are both closed subsets of $Z_j$. By the irreducibility of $Z_j$, we must have that:
	...
	\begin{myIndent}\HexOne
		...
		Note that for any ideal $I \lhd A$ we have that $V(I) = V(\sqrt{I})$. After all, since $I \subseteq \sqrt{I}$ we know that $V(\sqrt{I}) \subseteq V(I)$. On the other hand, $V(I) \subseteq V(\bigcap_{P \in V(I)} P) = V(\sqrt{I})$. Due to this observation, we can replace each ideal $I_n$ defining our closed sets with $\sqrt{I_n}$.\retTwo
	...
	\begin{myIndent}\HexOne
		...
		Now for every prime $P$ containing $I$ there is a minimal prime $P_i/I$ with $P_i/I \subseteq P/I$. Hence, we can conclude that $I \subseteq P_i \subseteq P$. Or in other words $\bigcup_{i=1}^n V(P_i) \subseteq V(I)$. The other containment is obvious.\retTwo
...
\begin{myIndent}\HexOne
	$(\Longrightarrow)$\\
	Suppose $A$ is a UFD. Then we can write $f = up_1^{e_1}\cdots p_n^{e_n}$ where each $p_i$ is a pairwise\\ nonassociate prime and each $e_i$ is a positive integer. But in turn, we know that\\ $\langle f\rangle = \prod_{i=1}^n \prod_{j=1}^{e_i}\langle p_i\rangle$ where the latter is a product of prime ideals. Hence, the minimal prime ideals over $\langle f\rangle$ must be contained among the $\langle p_i\rangle$. And this proves that every prime ideal minimal over $\langle f\rangle$ is principal.
	\begin{myIndent}\HexTwoP
	...
	Fortunately, note that $p$ is an irreducible element of $A$ if and only if $\langle p\rangle \neq A$,\\ $\langle p \rangle \neq \{0\}$, and $\langle p\rangle$ is maximal among principal proper ideals. But then since every prime ideal minimal over $\langle p\rangle$ is principal, we in turn must have that every minimal prime ideal over $\langle p\rangle$ is contained in $\langle p\rangle$. Since $\langle p\rangle$ does have a minimal prime over it (since $A/\langle p\rangle$ is nontrivial), the only way this makes sense is if $\langle p\rangle$ is the unique minimal prime ideal over itself. And now we've proven that $p$ is prime. $\blacksquare$\retTwo
\end{myIndent}
...
\exTwo\ul{Proposition:} If % https://q.uiver.app/#q=
$\begin{tikzcd}
...
\begin{itemize}
	...
	\item $\min(\alpha, \infty) = \alpha$ for all $\alpha \in \Gamma \cup \{\infty\}$.\retTwo
\end{itemize}
...
\begin{enumerate}
	...
	\begin{myIndent}\HexOne
		Note that $0 \in A$ and $v(0) = \infty \geq 1_\Gamma$. Meanwhile, suppose $x \in K^\times$. Then $v(x) = xU \geq 1U = 1_\Gamma$ iff $x = x \cdot 1^{-1} \in A$.\retTwo
	\end{myIndent}
...
\begin{enumerate}
	...
	\begin{myIndent}\HexOne
		...
		\begin{myIndent}\HexTwoP
			$(\Longrightarrow)$\\
			Suppose $S \subseteq \Gamma^\geq$ is a nonempty set. Since $A$ is Noetherian, we know the set $\{I_g : g \in S\}$ has a maximal ideal $I_{g_0}$. At the same time though, since $S$ is a chain in $\Gamma$ and $I_{g} \subseteq I_{g^\prime}$ iff $g^\prime \leq g$ we know that $\{I_g : g \in S\}$ is a chain with respect to set inclusion. Hence, $I_{g_0}$ contains $I_g$ for all $g \in S$, which in turn implies that $g_0 \leq g$ for all $g \in S$. This proves that $S$ has a minimal element $g_0$.\retTwo
		...
		\begin{myIndent}\HexTwoP
			...
			Equivalently, we have that $0_\Gamma < N1_\Gamma - g \leq 1_\Gamma$. And since $1_\Gamma$ is by\\ definition the minimal element of $\Gamma$ greater than $0_\Gamma$, we can conclude that\\ $N1_\Gamma - g = 1_\Gamma$. This is a contradiction as it implies that $(N - 1)1_\Gamma = g$. $\blacksquare$\retTwo
		\end{myIndent}
	\end{myIndent}
\end{enumerate}
...
Furthermore, if $A$ is a local PID that is not a field and $x$ generates the unique maximal ideal $P$ of $A$, then the unique factorization of any $a \in A - \{0\}$ into a product of irreducibles has form $a = ux^n$ where $u$ is a unit and $n \in \mathbb{Z}_{\geq 0}$.
\begin{myIndent}\color{orange}
	Why?\\
	Since $A$ is a PID, every irreducibe element is prime and every prime element generates an ideal which is maximal over all principal ideals (and hence all ideals).  of $A$ must be maximal over all pricipal ideals. It follows that up to companion class, $x$ is the unique irreducible element of $A$. In turn, the stated fact is just a result of $A$ also being a UFD.\retTwo
\end{myIndent}
...
\ul{Remark 5:} The prime spectrum of a DVR $A$ consists of only the unique maximal ideal and the zero ideal. This captures the idea that a DVR is the simplest type of PID that is not a field.
