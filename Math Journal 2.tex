...
\exTwo\ul{Theorem:} \hypertarget{Noetherian Rings allow prime factorization of ideals}{Let} $A$ be a Noetherian ring.
\begin{enumerate}
...
\begin{myIndent}\exThreeP
	...
	Next, suppose $k = \overline{k}$. Then $P \in \Spec(k[x_1, \ldots, x_n])$ implies $k[x_i] \cap P \in \Spec(k[x_i])$ for all $i$. Also, as $k[x_i]$ is a PID we can then conclude there is an irreducible polynomial $f \in k[x_i]$ such that $P \cap k[x_i] = \langle f\rangle$. Yet since $k$ is algebraically closed, the only way $f$ can be irreducible is if $f(x_i) = x_i - a_i$ for some $a_i \in k$.\retTwo
...
Let $A$ be a ring. The \udefine{Jacobson radical} of $A$ is $J(A) = \bigcap_{P \in \Max(A)} P$. Clearly\\ $\Nil(A) = \bigcap_{P \in \Spec(A)}P \subseteq J(A)$. Usually this containment will be proper but in\\ a special class of rings we have that $J(A) = \Nil(A)$.\retTwo
...
This next section will be on a completely different topic. For this section, we'll assume $A$ is Noetherian throughout.\retTwo
...
\exTwo\ul{Lemma:} Let % https://q.uiver.app/#q=WzAsNSxbMCwwLCJcXHswXFx9Il0sWzEsMCwiTV8xIl0sWzIsMCwiTV8yIl0sWzMsMCwiTV8zIl0sWzQsMCwiXFx7MFxcfSJdLFsxLDIsIlxccGhpXzEiXSxbMiwzLCJcXHBoaV8yIl0sWzMsNF0sWzAsMV1d
$\begin{tikzcd}
	{\{0\}} & {M_1} & {M_2} & {M_3} & {\{0\}}
...
\begin{myIndent}\exThreeP
	Proof:\\
	To start off $\phi_1(M_1)$ is a submodule of $M_2$. Hence any submodules of $M_1$ are isomorphic to submodules of $M_1$. In particular, if $M_1$ has a submodule isomorphic to $A/P$ for some $P \in \Spec(A)$, then so does $M_2$. Hence, $\Ass(M_1) \subseteq \Ass(M_2)$.\retTwo
...
\exTwo\ul{Proposition:} Let $M$ be a nontrivial finitely generated module over a Noetherian ring $A$.
\begin{itemize}
...
\pracOne One application of this proposition is as follows. Suppose $A$ is a \udefine{reduced} (meaning\\ $\Nil(A) = \{0\}$) Noetherian ring.
\begin{myIndent}\pracTwo
	We already know that minimal primes consist exclusively of zero divisors. After all, if $A$ is an integral domain then the only minimal prime is $\{0\}$. Meanwhile, if $A$ not an integral domain then we know every prime ideal of $A$ contains nonzero elements. Yet simultaneously, because $A$ is Noetherian, we can apply the theorem on \inLinkRap{Noetherian Rings allow prime factorization of ideals}{pages 899-900} plus the fact that every prime ideal contains a minimal prime in order to say that $0$ is a product of every minimal prime ()
\end{myIndent}
