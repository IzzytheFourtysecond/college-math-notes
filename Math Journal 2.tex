...
Now a \udefine{Riemann surface} is defined as a $2$-dimensional topological manifold $M$ equipped with a complex structure. I will refer to all the charts in the complex structure on $M$ as \udefine{complex charts}.\retTwo
...
Based on all the prior reasoning, we can now easily see that $\mathbb{C}/\Lambda$ is a topological manifold. We define a complex structure generated by the maps $\pi: V \to \mathbb{C}/\Lambda$ where $\pi$ is a homeomorphism from $V \subseteq \mathbb{C}$ into $\mathbb{C}/\Lambda$. The transition maps between the charts corresponding to these maps will all have the form $z \mapsto z + n\omega_1 + m\omega_2$ for some $n, m \in \mathbb{Z}$. So this is a well-defined complex structure.\retTwo 

One other note I'd like to make is that by the universal property of quotient maps, we have that any function on $\mathbb{C}/\Lambda$ is associated with a doubly periodic function on $\mathbb{C}$ (i.e. $f(z) = f(z + \omega_1) = f(z + \omega_2)$).\retTwo
...
\begin{myIndent}\pracTwo
	...
	Next let $\{U_\alpha\}_{\alpha \in A}$ be any open cover of $\mathbb{C}/\Lambda$. Then we have that $\{\pi^{-1}(U_\alpha)\}_{\alpha \in A}$ is an open cover of $\mathbb{C}$ and in turn also of $K$. Since $K$ is compact, we can find $\alpha_1, \ldots, \alpha_n$ with $K \subseteq \bigcup_{j=1}^n \pi^{-1}(U_{\alpha_j}) = \pi^{-1}(\bigcup_{j=1}^n U_{\alpha_j})$. And finally since $\pi$ is surjective, we have that $\pi(\pi^{-1}(\bigcup_{j=1}^n U_{\alpha_j})) = \bigcup_{j=1}^n U_{\alpha_j}$. So, $\mathbb{C}/\Lambda = \pi(K) \subseteq \bigcup_{j=1}^n U_{\alpha_j}$.\retTwo
\end{myIndent}
...
\begin{myIndent}
	Suppose $M$ and $N$ are Riemann surfaces. Then a map $F: M \to N$ is \udefine{holomorphic} if for all $p \in M$ there is a complex chart $(U, \varphi)$ on $M$ and a complex chart $(V, \psi)$ on $N$ such that $p \in U$, $F(U) \subseteq V$, and $\psi \circ F \circ \varphi^{-1}$ is a holomorphic function (in the complex analytic sense) from $\varphi(U)$.\newpage
\end{myIndent}
