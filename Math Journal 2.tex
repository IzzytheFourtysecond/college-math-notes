...
\begin{myIndent}\HexOne
	...
	\begin{myIndent}\HexPPP
		Note that $\sum_{j \in k_1 H^s}\zeta_{13}^j \neq \sum_{j \in k_2 H^s}\zeta_{13}^j$ if $k_1 H^s \neq k_2 H^s$ since otherwise would contradict the $\mathbb{Q}$-linear independence of $1, \zeta_{13}, \zeta_{13}^2, \ldots, \zeta_{13}^{12}$.\retTwo
	\end{myIndent}
...
\begin{enumerate}
	...
	\begin{myIndent}\HexOne
		...
		By part (a), we thus know that $S$ is a disjoint union zero sets of certain irreducible\\ degree $n$ polynomials. Finally, as every irreducible polynomial in $\mathbb{F}_p$ is separable (or\\ alternatively because our polynomials divide the separable polynomial $x^{p^n} - x$),\\ we can conclude that each zero set has $n$ elements. This proves that:
...
\hTwo We can also use the following lemma to come up with a useful construction.\retTwo

\exTwo\ul{Lemma:} If $\varphi_1, \varphi_2$ is subharmonic then so is $\varphi \coloneqq \max(\varphi_1, \varphi_2)$.
\begin{myIndent}\exThreeP
	Proof:\\
