...
\exTwo\ul{Lemma 1:} \hypertarget{Lemma 1 pages 857-858}{Suppose} $F, F^\prime$ are fields, $\theta : F \to F^\prime$ is a field isomorphism, $K/F$ is an algebraic extension, and $L/F$ is another extension such that $L$ is algebraically closed. Then there exists a field homomorphism $\phi \in \Embed_{\theta}(K, L)$.
\begin{myIndent}\exThreeP
	Proof:\\
