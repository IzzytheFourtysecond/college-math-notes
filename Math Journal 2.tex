...
\exTwo\ul{Theorem (Invariance of Domain):} Suppose $U \subseteq \mathbb{R}^n$ is open and $f: U \to \mathbb{R}^n$ is an injective continuous function. Then $f(U)$ is an open set.
\begin{myIndent}\exThreeP
...
\hTwo Based on the prior corollary, we can now easily conclude that the dimension of a\\ topological manifold is well-defined. In fact, we can make an even stronger statement that if $M$ is an $n$-dimensional topological manifold and $N$ is an $m$-dimensional topological manifold, then $M$ being homeomorphic to $N$ implies that $m = n$. Thus, the dimension of a topological manifold is invariant under homeomorphisms.\retTwo 

\exTwo\ul{Proposition:} Suppose $M$ is an $n$-dimensional  topological manifold. Then $x$ is a boundary point of $M$ if and only if there is a chart $(U, \varphi)$ on $M$ such that $x \in U$, $\varphi(U)$ is open in $H^n$, and $\varphi(x) \in \partial H^n$.\newpage
\begin{myIndent}\exThreeP
	$(\Longrightarrow)$\\
	The definition of a boundary point that I gave on \inLinkRap{placeholder}{page 146} is that there are no charts\\ $(V, \psi)$ on $M$ with $x \in V$ and $\psi(V)$ open in $\mathbb{R}^n$. So suppose $x$ is a boundary point. By the definition of a topological manifold there must exists a chart $(U, \varphi)$ on $M$ with $x \in U$. But since $x$ is a boundary point, we know that $\varphi(U)$ is not open in $\mathbb{R}^n$. Hence, we know that $\varphi(U)$ is only open in $H^n$. Furthermore, by intersecting $\varphi(U)$ with balls of decreasing radius around $\varphi(x)$, we can't be able to get a restricted chart whose image is open in $\mathbb{R}^n$. Hence, it follows that $\varphi(x) \in \partial H^n$.\retTwo
