...
\item[(b)] Now for the next part, Folland asks us to prove that if the $L^2$ derivative $f^\prime$ exists, then:\\ [-2pt] $\left[\int |f(x)|^2 \df x\right]^2 \leq 4 \int |xf(x)|^2 \df x \int |f^\prime(x)|^2 \df x$. Furthermore, his hinted solution involves\\ [2pt] evaluating $\int |f(x)|^2 \df x$ using integration by parts. But that would require knowing that $f^\prime$ is\\ [2pt] continuous, which we don't know. Also, it's possible for the inequality he wrote down to not\\ [2pt] be well defined if $\int |f^\prime(x)|^2 \df x = 0$ but $\int |xf(x)|^2 \df x = \infty$. In an effort to address this, I will assume for this part that $f$ is a Schwartz function.\newpage
