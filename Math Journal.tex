...
\blab{Tensors:}\\
Let $V$ be an $n$-dimensional vector spaces over the field $F$. Let $V^k$ be the set of all $k$-tuples of elements of $V$. Then a function $T: V^K \to F$ is said to be \udefine{linear in its $i$th. variable} if for all $u, v_1, \ldots, v_k \in V$ and $a, b \in F$, we have that:
...
Let $V$ be an $n$-dimensional vector space over the field $F$ and $S_k$ be the symmetric group over $\{1, \ldots, k\}$. For $\sigma \in S_k$ and $T \in \mathcal{L}^k(V)$, we define $T^\sigma \in \mathcal{L}^k(V)$ by:
...
\begin{itemize}
	...
	\item[(d)] The map $\Alt: \mathcal{L}^k(V) \to \mathcal{L}^k(V)$ defined by $T \mapsto \Alt(T)$ is a linear map.
	
	\begin{myIndent}\exTwoP
		Proof:\\
		$\Alt$ is a linear map because it is a linear combination of a bunch of linear maps.\retTwo
	\end{myIndent}
\end{itemize}
...
Let $u_1, \ldots, u_n$ be a basis of the vector space $V$ over a field $F$, and let $u_1^*, \ldots, u_n^*$ be the corresponding dual basis. Now given the multi-index $I = (i_1, \ldots, i_k)$, set $u_I^* = u_{i_1}^* \otimes \cdots \otimes u_{i_k}^*$. Next define $\Psi_I = \Alt(u_I^*)$.\retTwo
...
\begin{itemize}
	...
	\item[(c)] If $I$ and $J$ are strictly increasing, then $\Phi_I(u_{j_1}, \ldots, u_{j_k}) = \delta_{I,J}$ where $\delta$ is the Kronecker delta function.
