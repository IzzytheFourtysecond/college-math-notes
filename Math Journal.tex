...
\ul{Theorem:} Consider any measure space $(X, \mathcal{M}, \mu)$ (where $\mu$ is positive), let $U \subseteq \mathbb{R}^n$ be an open set, and suppose that $f: X \times U \to \mathbb{C}$ is a function such that $f(\cdot, y): X \to \mathbb{C}$ is integrable for all $y \in U$. Also define $F(y) = \int_X f(x, t)\df \mu(x)$.
...
\Hstatement\blab{Exercise 7.26:} Suppose $X$ is an LCH space. If $\{\mu_k\}_{k \in \mathbb{N}} \subseteq M(X)$, $\mu_k \to \mu$ vaguely, and\\ $\|\mu_k\| \to \|\mu\|$, then $\int f \df \mu_k \to \int f \df \mu$ for every $f \in BC(X)$.
...
{\center\includegraphics[scale=1]{Folland_screenshot.png}\retTwo\par}

Also, given some proposition $P$, probabilists typically write $\{P(\omega) = \text{true}\}$ as\\  opposed to $\{\omega \in \Omega : P(\omega) = \text{true}\}$. As long as I'm studying probability theory,\\ I'm going to use probability terminology and notation.\retTwo
...
If $X \in L^1 \cap L^2$, then $E((X - a)^2) = E(X^2 - 2aX + a^2) = E(X^2) - 2aE(X) + a^2$. Note that this is a quadratic which attains it's minimum at $a = E(X)$.\newpage 
...
Hence, for all $X \in L^1 \cap L^2$ we have that:

{\centering$\sigma^2(X) = E(X^2) - 2(E(X))^2 + (E(X))^2 = E(X^2) - (E(X))^2$.\retTwo\par}

Let $(\Omega, \mathcalli{B}, P)$ be a sample space and let $(\Omega^\prime, \mathcalli{B}^\prime)$ be another measurable space. Given a $(\mathcalli{B}, \mathcalli{B}^\prime)$-measurable map $\phi$, we may define the \udefine{image measure} $P_\phi(E) \coloneqq P(\phi^{-1}(E))$ on $(\Omega^\prime, \mathcalli{B}^\prime)$.
