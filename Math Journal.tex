...
\begin{myIndent}\exThreeP 
	...
	\begin{myIndent}\exPPP
		Specifically consider the compact set $K \coloneqq \pi_X(\supp(f)) \times \pi_Y(\supp(f)) in X \times Y$. Then $\supp(f) \subseteq K$, $K$ is compact, and $\mu \times \nu(K) = \mu(\pi_X(\supp(f)))\nu(\pi_Y(\supp(f)))$. Also, the latter expression is finite since $\pi_X(\supp(f))$ and $\pi_Y(\supp(f))$ are compact and $\mu$ and $\nu$ are Radon.\retTwo
	\end{myIndent}

	This proves that every $f \in C_c(X, Y)$ is also in $L^1(\mu \times \nu)$. Finally, even if $\mu$ and $\nu$ are not $\sigma$-finite, we can still show that Fubini's theorem holds. Specifically, consider taking\\ $\mu^\prime(E) \coloneqq \mu(E \cap \pi_X^{-1}(\supp(f)))$ and $\nu^\prime(E) \coloneqq \nu(E \cap \pi_Y^{-1}(\supp(f)))$.\newpage
