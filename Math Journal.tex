...
\begin{myIndent}\HexOne
	...
	Now by Khinchine's strong law of large numbers, we know that $\frac{1}{n}\sum_{j=1}^n X_j^2 \to \sigma^2 = 1$\\ [1pt] almost surely as $n \to \infty$. Also, by a straightfoward application of the prior exercise using\\ $\phi(z) = \sqrt{\rea{z}}\chi_{(0, \infty)}(\rea{z})$, we know that $(\frac{1}{n}\sum_{j=1}^n X_j^2)^{1/2} \to \sqrt{1} = 1$ almost surely\\ as $n \to \infty$.\newpage 
...
\begin{itemize}
	...
	\begin{myIndent}\exThreeP
		...
		Similarly, we have for any $A \in \mathcalli{B}_\mathbb{C}$ and $y \in Y$ that $(f^y)^{-1}(A) = (f^{-1}(A))^y$. Thus if $f$ is $\mathcalli{B}_{X \times Y}$-measurable, then it is a straightfoward application of part (a) to prove part (b). $\blacksquare$\newpage
	\end{myIndent}
\end{itemize}
...
\begin{myIndent}\exThreeP
	...
	\begin{enumerate}
		...
		\begin{myIndent}\exPPP
			Why? This is a straightfoward application of the finite additivity of measures and\\ integrals and also the fact that a finite sum of Borel measurable functions is also Borel measurable.\retTwo 
		\end{myIndent}
...
\begin{myIndent}\hThree
	...
	\hThree Now a question one may ask is whether this definition of boundedness is compatible with our definition of boundedness on metric spaces. Unfortunately, the answer is generally no. For example, consider $\mathbb{C}$ as a complex vector space and then equip $\mathbb{C}$ with the metric $\rho(z_1, z_2) \coloneqq \min(|z_1 - z_2|, 1)$.

	\begin{myIndent}\fontsize{11}{13}\color{Orange}
		Sanity check:\\
		Note that $\rho$ is in fact a metric. After all, clearly $\rho(z_1, z_2) = 0$ iff $|z_1 - z_2| = 0$ and the latter happens iff $z_1 = z_2$. And otherwise we have that $\rho(z_1, z_2) > 0$. Also note that clearly $\rho(z_1, z_2) = \rho(z_2, z_1)$. Finally, suppose $z_1, z_2, z_3 \in \mathbb{C}$. If $|z_1 - z_2| + |z_2 - z_3| \leq 1$, then we must have that $|z_1 - z_1| \leq 1$ and $|z_2 - z_3| \leq 1$. Hence:

		{\centering $\min(|z_1 - z_2| + |z_2 - z_3|, 1) = |z_1 - z_2| + |z_2 - z_3| = \rho(z_1, z_2) + \rho(z_2, z_3)$. \retTwo\par}

		On the other hand, if $|z_1 - z_2| + |z_2 - z_3| > 1$, then we must have that:
		
		{\centering\begin{tabular}{l}
			$1 \leq \min(|z_1 - z_2| + |z_2 - z_3|, |z_1 - z_2| + 1, |z_2 - z_3| + 1, 2)$\\ [2pt]
			
			$\phantom{aaaaaaaaaaaaaaaaa} \leq \min(|z_1 - z_2|, 1) + \min(|z_2 - z_3|, 1) = \rho(z_1, z_2) + \rho(z_2, z_3)$
		\end{tabular}\retTwo\par}

		Either way this proves the triangle inequality since:
		
		{\centering$\rho(z_1, z_3) = \min(|z_1 - z_3|, 1) \leq \min(|z_1 - z_2| + |z_2 - z_3|, 1) \leq \rho(z_1, z_2) + \rho(z_2, z_3)$.\retTwo\par}
	\end{myIndent}
	
	Then it is easy to see that the entire space $\mathbb{C}$ is bounded with respect to $\rho$ according to the metric space definition of boundedness. Also, it is easy to see that $\rho$ generates the same topology on $\mathbb{C}$ as the Euclidean metric. Hence, $(\mathbb{C}, \rho)$ is a complex topological vector space. That said, if $B$ is the open unit ball about $0$, then clearly $B$ is balanced and contains $0$ but there does not exist any $r \in \mathbb{C}$ such that $\mathbb{C} \subseteq rB$. So $\mathbb{C}$ is not von Neumann bounded in the topology induced by $\rho$.\retTwo
...
\hTwo There are a few more things I'd like to take notes on from this lecture. However, it's now 9/28/2025 and I need to make sure I have my notes done for my other math classes. So I'll come back to my functional analysis notes later on \inLinkRap{math 241a lecture 2}{page \_\_\_}.
...
\begin{myIndent}\hThree
	...
	\begin{myIndent}\pracTwo
		...
		\begin{itemize}
			...
			\item Meanwhile, suppose $V \in \mathcal{T}$ is an open set containing $z$. Then by restricting $V$ to $\mathbb{C}$ we can assume $V \subseteq \mathbb{C}$. So, there is a (Euclidean) ball $B \subseteq \mathbb{C}$ of radius $\varepsilon > 0$ centered at $z$ which is contained in $V$.\retTwo
		...
		\begin{itemize}
			...
			So, $U = \{z \in \mathbb{C} : |z| > \sfrac{2}{\varepsilon}\} \cup \{\infty\}$ is an open set in $\mathcal{T}$ with $\infty \in U \subseteq  V$.\retTwo
		\end{itemize}
...
\hypertarget{math 200a lecture 3}{}
...
\begin{enumerate}
	...
	\item \hypertarget{bib citation 12}{Garrett}, P. (2014, April 23). \textit{Seminorms and locally convex spaces}. Retrieved September 26, 2025, from https://www-users.cse.umn.edu/\~garrett/m/fun/notes\_2012-13/07b\_seminorms.pdf
