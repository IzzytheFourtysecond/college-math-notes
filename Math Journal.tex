...
Now, it's unfortunate that this extension of the Lebesgue-Radon-Nikodym theorem loses the ability to decompose $\nu$ into a continuous part and a mutually singular part. That said, an extremely convenient fact which makes the last theorem feel more worthwhile is that it turns out that every positive Radon measure is decomposable.\retTwo

Let $X$ be a LCH space and let $\mu$ be a Radon measure on $(X, \mathcalli{B}_X)$. Then recall from the proof of the Riesz Representation thorem in my math 240c notes that if $\mu^*(E) = \inf \{\mu(U) : U \supseteq \text{ with } U \text{ open}\}$, then $\mu^*$ is a well-defined outer measure 

\exTwo\ul{Lemma 19.30:}
