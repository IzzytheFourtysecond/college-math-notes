...
\exOne\blab{Exercise 3:} Let $J$ and $E$ be well-ordered sets, and suppose there is an order-preserving map $k: J \longrightarrow E$. Using exercises 1 and 2, show that $J$ has the order type of $E$ or a section of $E$.\retTwo
...
\begin{myIndent}\exTwoP
   ...
   Let $J_0 = \{\alpha \in J \mid h(\alpha)\leq k(\alpha)\}$. Then suppose that for some $\alpha \in J$, we have that $S_\alpha \subseteq J_0$. Because $k$ is order preserving, we know that $k(\alpha) > k(\beta) \geq h(\beta)$ for all $\beta \in S_\alpha$. Hence, $k(\alpha) \notin h(S_\alpha)$, meaning that $h(S_\alpha) \neq E$. So, we conclude that $h(\alpha) = \text{smallest}(E - h(S_\alpha))$\retTwo
   
   Now note that:
   
   {\centering\fontsize{11}{13}\selectfont
      $k(\alpha) \geq \text{smallest}(E - \hspace{-0.4em}\bigcup\limits_{\beta \in S_\alpha}\hspace{-0.4em}S_{k(\beta)}) \geq \text{smallest}(E - \hspace{-0.4em}\bigcup\limits_{\beta \in S_\alpha}\hspace{-0.4em}S_{h(\beta)}) \geq \text{smallest}(E - h(S_\alpha)) = h(\alpha)$\retTwo\par}
   
   Therefore, $\alpha \in J_0$. By transfinite induction, this proves that $J = J_0$. Also, as is hopefully clear from the above proof, $S_\alpha \subseteq J_0 \Longrightarrow E - h(S_\alpha) \neq \emptyset$. So $h(\alpha)$, will never be determined by the second case of our definition above.\retTwo

   By exercise 2, we know that $h$ is an order-preserving map whose image is either $E$ or a section of $E$. Thus, $J$ has the same order type as exactly one of either the entirety of $E$ or a section of $E$.\retTwo
\end{myIndent}

\blab{Exercise 4:} Use exercises 1-3 to prove the following:

\begin{itemize}
   \item[(a)] If $A$ and $B$ are well-ordered sets, then exactly one of the following three\\ conditions holds: $A$ and $B$ have the same order type, $A$ has the order type of\\ a section of $B$.
   
   
   \begin{myIndent}
      So the hinted route for solving this problem using an exercise I didn't do. And right now I really don't want to do that exercise. So I'm just going to write out the thing I was supposed to have proven earlier.
   \end{myIndent}
   
\end{itemize}

% If A and B are well-ordered sets, then exactly one of the following three
...
% (b) Suppose that A and B are well-ordered sets that are uncountable, such that
