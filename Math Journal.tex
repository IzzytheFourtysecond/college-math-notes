...
\Hstatement\blab{Exercise 2.45:} Let $n \geq 3$ and suppose $(X_j, \mathcal{M}_j, \mu_j)$ is a measure space for $j = 1, \ldots, n$, and let us identify $\prod_{j=1}^n X_j$ with $(\prod_{j=1}^{n-1} X_j) \times X_n$  and $X_1 \times (\prod_{j=2}^{n} X_j)$ in the obvious ways.
\begin{itemize}
...
Also, if $f \in L^1(\nu_1 \times \nu_2)$, then we can easily see via the Fubini-Tonelli theorem that:
...
\begin{myIndent}\pracOne
	...
	Now showing that $\mu * \nu$ is a countably additive measure is as simple as noting that\\ $\alpha^{-1}(\emptyset) = \emptyset$ and $\alpha^{-1}(\bigcup E_n) = \bigcup \alpha^{-1}(E_n)$. Meanwhile, by theorem 7.8 in my math 240c notes, it suffices to show that $|\mu * \nu|(K) < \infty$ for all compact sets $K$ in order to say that $\mu * \nu$ is regular (and thus Radon). Yet this is trivially true for any complex measure.
...
\begin{itemize}
	...
	\item[(b)] For any bounded Borel measurable function $h$, $\int h \df (\mu * \nu) = \iint h(x + y)\df \mu(x) \df \nu(y)$.
	
	\begin{myIndent}\exThreeP
		Proof:\\
		This clearly holds for characteristic functions and in turn by linearity it holds for simple functions.\retTwo

		Next if $f$ is measurable and nonnegative, then letting $(\phi_n)_{n \in \mathbb{N}}$ be a monotonically increasing sequence of nonnegative simple functions converging to $f$ pointwise, we have by the monotone convergence theorem that:

		{\centering\begin{tabular}{l}
			$\int f \df(\mu * \nu) = \lim_{n \to \infty} \int \phi_n \df(\mu * \nu)$\\ [4pt]
			$\phantom{\int f \df(\mu * \nu)} = \lim_{n \to \infty} \iint \phi_n(x + y)\df \mu(x)\df \nu(y)$\\ [4pt]
			$\phantom{\int f \df(\mu * \nu)} = \iint \lim_{n \to \infty} \phi_n(x + y) \df \mu(x)\df \nu(y) = \iint f(x + y)\df \mu(x) \df \nu(y)$.
		\end{tabular}\retTwo\par}
	\end{myIndent}
	...
	\item[(d)] If $\mu = f \df m$ and $\nu = g \df m$, then $\mu * \nu = (f * g)\df m$; that's to say, if we view $L^1(m)$ as a subspace of $M(X)$ via the mapping $f \mapsto f \df m$ (where $m$ is the Lebesgue measure), then our old and new definition of convolutions coincide.
