...
\begin{myIndent}
	...
	{\centering $\int_M f \df V = \sum\limits_{i=1}^\infty \int_{V_i} f\phi_i \df V_{\alpha_i} = \sum\limits_{i=1}^\infty \int_{U_i} (f\phi_i \circ \alpha_i) V(\Df \alpha_i) \df m$ \retTwo\par}
	...
	\exTwo\ul{Lemma:} A subset $E \subseteq M$ has measure zero if and only if $E$ can be covered by a collection of coordinate patches $\{\beta_j: U_j^\prime \to V_j^\prime\}_{j \in \mathbb{N}}$ such that $\beta_j^{-1}(V_j \cap E)$ has\\ Lebesgue measure zero for all $j$.

	\begin{myIndent}\exTwoP
		$(\Longrightarrow)$\\ Let $\{\alpha_i: U_i \to V_i\}$ be a covering of coordinate patches on $M$ and let\\ $\{\phi_i: \mathbb{R}^k \to \mathbb{R}\}_{i \in \mathbb{N}}$ be a partition of unity such that $\supp(\phi_i) \cap M \subseteq V_i$ for\\ all $i$. Then since $V(E) = 0$, we know that:

		{\centering $0 = \sum\limits_{i=1}^\infty \int_{V_i \cap E} \phi_i \df V_{\alpha_i} = \sum\limits_{i=1}^\infty \int_{\alpha_i^{-1}(V_i \cap E)} (\phi_i \circ \alpha_i) V(\Df \alpha_i)\df m$ \retTwo\par}

		And because the latter integrals are all nonnegative, we know for all $i \in \mathbb{N}$ that, $\int_{\alpha_i^{-1}(V_i \cap E)} (\phi_i \circ \alpha_i) V(\Df \alpha_i)\df m = 0$.\retTwo
		
		Next, setting $N_i = \{x \in V_i : \phi_i(x) > 0\}$, we know by the continuity of $\alpha_i$ and $\phi_i$ that $N_i$ and $\alpha_i^{-1}(N_i)$ are open. Hence, $\beta_i \coloneqq \alpha_i|_{\alpha_i^{-1}(N_i)}$ is a coordinate patch satisfying that:
		
		{\centering $\int_{\alpha_i^{-1}(V_i \cap E)} (\phi_i \circ \alpha_i) V(\Df \alpha_i)\df m = \int_{\beta_i^{-1}(N_i \cap E)} (\phi_i \circ \beta_i) V(\Df \beta_i)\df m$ \retTwo\par}

		Since $(\phi_i \circ \beta_i) V(\Df \beta_i) > 0$ on $\beta_i^{-1}(N_i \cap E)$, we thus must have that that\\ $m(\beta_i^{-1}(N_i \cap E)) = 0$. Also, it's easy to see that $\{\beta_i: \alpha_i^{-1}(N_i) \to N_i\}_{i \in \mathbb{N}}$ is a collection of coordinate patches covering all of $M$ (although $\{\phi_i\}_{i \in \mathbb{N}}$ is no longer subordinate to it necessarily). After all, for all $p \in M$ there is some $i$ such that $\phi_i(p) > 0$. Hence, the $N_i$ cover all of $M$.\retTwo

		$(\Longleftarrow)$\\
		Note that we can consider $\bigcup_{i \in \mathbb{N}} V_i^\prime$ to be a manifold and construct a partition of unity $\{\psi_i\}_{i \in \mathbb{N}}$ subordinate to the covering $\{\beta_i: U_i^\prime \to V_i^\prime\}_{i \in \mathbb{N}}$ of that\\ manifold.\newpage
		
		Now if $\{\alpha_i: U_i \to V_i\}_{i \in \mathbb{N}}$ and $\{\phi_i\}_{i \in \mathbb{N}}$ are as in the previous proof's direction we have that:

		$V(E) = \sum\limits_{i=1}^\infty \int_{V_i \cap E} \phi_i \df V_{\alpha_i} = \sum\limits_{i=1}^\infty \int_{V_i \cap E} \phi_i\left(\sum\limits_{j=1}^\infty \psi_j \chi_{V_i^\prime}\right) \df V_{\alpha_i}$
