...
% \item[(b)] Now for the next part, Folland asks us to prove that if the $L^2$ derivative $f^\prime$ exists, then:\\ [0pt] $\left[\int |f(x)|^2 \df x\right]^2 \leq 4 \int |xf(x)|^2 \df x \int |f^\prime(x)|^2 \df x$. I think this is wrong. Furthermore, his\\ hinted solution involves evaluating $\int |f(x)|^2 \df x$ using integration by parts. But that would\newpage require knowing that $f^\prime$ is continuous, which we don't know. Also, it's possible for that\\ inequality he wrote to not be well defined if $\int |f^\prime(x)|^2 \df x = 0$ but $\int |xf(x)|^2 \df x = \infty$. Not to mention, I think I might have thought of a counter example using exercise 3.40 in\\ Folland's textbook (I haven't done that exercise yet though but I'll try to get around to it some time this Summer). In an effort to address this, I will assume for this part that $f$ is a Schwartz function.\newpage

% \begin{myIndent}\HexOne
% 	Our problem is trivial if $\int |xf(x)|^2 \df x = \infty$. So assume the right hand side is finite:


% 	\newpage
% \end{myIndent}

% \item[(b)] If the $L^2$ derivative $f^\prime$ exists, then: $\left[\int |f(x)|^2 \df x\right]^2 \leq 4 \int |xf(x)|^2 \df x \int |f^\prime(x)|^2 \df x$ (where we interpret $\infty \cdot 0$ on the right side of the equation as $\infty$ if it ever comes up). 


% \item[(c)] (\blab{Heisenberg's Inequality}) For any $b, \beta \in \mathbb{R}$,

% {\centering $\int (x-b)^2|f(x)|^2\df x\int (\xi - \beta)^2|\widehat{f}(\xi)|^2\df \xi \geq \frac{\|f\|_2^4}{16\pi^2}$ \retTwo\par}
