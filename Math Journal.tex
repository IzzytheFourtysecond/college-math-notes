...
\begin{myIndent}\exThreeP
	...
	Now we can't have that $\mu(K) = 0$ since that would contradict that $\nu(K) = \int_K f \df \mu > 0$. Hence, there is a nonempty countable family $\mathcalli{F}_2 \subseteq \mathcalli{F}$ such that $\mu(F \cap K) > 0$ for all\\ $F \in \mathcalli{F}_2$ and $\mu(F \cap K) = 0$ for all $F \notin \mathcalli{F}_2$. Also, since $K$ doesn't intercept any $F \in \mathcalli{F}_1$, we know that $\mathcalli{F}_1 \cap \mathcalli{F}_2 = \emptyset$. But now note that:
	...
	\begin{myIndent}\exPPP
		And as a side note: if $f\chi_E \in L^1(\mu)$ because $\int f \chi_E \df \mu = \nu(X) < \infty$.\retTwo
	\end{myIndent}
...
An important application of the above fact is that if $m$ is the Lebesgue measure on $\mathbb{R}^n$, then we can identify $L^1(m)$ as a subspace $M(X)$. This will be used in my journal later.\retTwo
...
\exTwo\ul{Proposition 7.19:} Suppose $\mu, \mu_1, \mu_2, \ldots \in M(\mathbb{R})$ and let $F_n(X) = \mu_n((-\infty, x])$ and\\ $F(x) = \mu((-\infty, x])$.\\ [-22pt]
\begin{itemize}
...
\Hstatement\blab{Exercise 2.45:} Suppose $(X_j, \mathcal{M}_j, \mu_j)$ is a measure space for $j = 1, 2, 3$, and let us identify $\prod_{j=1}^3 X_j$ with $(X_1 \times X_2) \times X_3$ in the obvious way.
\begin{itemize}
	\item We have that $\bigotimes_{j=1}^3 \mathcal{M}_j = (\mathcal{M}_1 \otimes \mathcal{M}_2) \otimes \mathcal{M}_3$.
	\begin{myIndent}
		\HexOne Proof:\\ 
		For $i = 1, 2, 3$ let $\pi_i$ be the projection of $\prod_{j=1}^3 X_j$ on $X_i$. Also let $\pi_{1,2}$ be the projection of $\prod_{j=1}^3 X_j$ onto $X_1 \times X_2$. Now since $\mathcal{M}_1 \otimes \mathcal{M}_2$ is generated by the basis $\{\pi_{\alpha}^{-1}_i(E) : i=1,2 \text{ and } E \in \mathcal{M}_i\}$
		
		
		Use proposition on page 13.
		
		Now $(\mathcal{M}_1 \otimes \mathcal{M}_2) \otimes \mathcal{M}_3$ and $\bigotimes_{j=1}^3 \mathcal{M}_j$ are the $\sigma$-algebras\\ [3pt] generated respectively by the collections:

		\begin{itemize}
			\item[\bullet] $A_1 \coloneqq \{\pi_{1,2}^{-1}(E) : E \in \mathcal{M}_1 \otimes \mathcal{M}_2\} \cup \{\pi_3^{-1}(E) : E \in \mathcal{M}_3\}$,
			\item[\bullet] $A_2 \coloneqq \{\pi_j^{-1}(E) : j=1,2,3 \text{ and } E \in \mathcal{M}_j\}$.\retTwo
		\end{itemize}

		By considering the sets $F = X_1 \times F_2$ and $E = E_1 \times X_2$ in $\mathcal{M}_1 \otimes \mathcal{M}_2$ for any $E_1 \in \mathcal{M}_1$ and $F \in \mathcal{M}_2$, it's clear that $A_2 \subseteq A_1$. Hence $\bigotimes_{j=1}^3 \mathcal{M}_j \subseteq (\mathcal{M}_1 \otimes \mathcal{M}_2) \otimes \mathcal{M}_3$.\retTwo
