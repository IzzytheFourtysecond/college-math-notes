...
\begin{myIndent}\HexOne
	...
	Now since $\nu$ is finite, it suffices to show that $\nu$ is regular on any set $E \in \mathcalli{B}_X$. Luckily, by corollary 3.6 (see my math 240a notes from Fall quarter), we know that given a fixed $\varepsilon > 0$ there exists $\delta > 0$ such that $\mu(A) < \delta$ implies that $|\nu(A)| = \nu(A) < \varepsilon$. Then by the outer regularity of $\mu$, we can pick an open set $U \supseteq E$ with $\mu(U - E) < \frac{\delta}{2}$.
\end{myIndent}
