...
{\begin{myIndent}\hTwo
   \uuline{Fact}: There is a one-to-one correspondence between the set of $m\times n$ real\\ matrices and $L(X, Y)$.\\ [-8pt]
   
   {\begin{myIndent}\hThree
      Firstly, let $\mMat{A} \in L(X, Y)$. Then we already saw above how to construct\\ a matrix $[\mMat{A}]$ from the linear mapping $\mMat{A}$.\retTwo
      
      Now observe if $\mVec{x} \in X$, then $\mVec{x} = \sum\limits_{j=1}^n c_j\mVecAst{x}_j$. Thus, because $\mMat{A}$ is linear:

      {\centering\fontsize{12}{14}\selectfont 
      \begin{tabular}{l}
         $\mMat{A}(\mVec{x}) = \mMat{A}\left(\sum\limits_{j=1}^n c_j\mVecAst{x}_j\right) = \sum\limits_{j=1}^n c_j\mMat{A}(\mVecAst{x}_j)$ \\ [2pt]
      ...
      Thus, we have an equation for $\mMat{A}(\mVec{x})$ in terms of the components of $[\mMat{A}]$.\\ Needless to say, if we were instead starting out with an $m \times n$ real\\ matrix $[\mMat{B}] \in \mathcal{M}_{m\times n}(\mathbb{R})$ with components $b_{i,j}$, then we could define the\\ linear map $\mMat{B} \in (L(X, Y))$ given by the rule:
