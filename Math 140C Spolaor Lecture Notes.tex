...
{\begin{myIndent}\hTwo
   \uuline{Theorem 9.23 (Banach Fixed Point Theorem)}:\\ [1pt] Let $X$ be a nonempty complete metric space with metric $d$, and let $\varphi: X \longrightarrow X$\\ be a contraction. Then there exists a unique $x \in X$ such that $\varphi(x) = x$\\ [-6pt]
...
{\begin{myIndent}\hTwo
   ...
   {\begin{myIndent}\hThree
      ...
      \begin{tabular}{l}
         ...
         $\phantom{\|\varphi(\mVec{x}) - \mVecAst{x}_0\|} < \frac{1}{2}\|\mVec{x} - \mVecAst{x}_0\| + \frac{R}{2} \leq \frac{R}{2} + \frac{R}{2} = R$
      \end{tabular}\retTwo\par}
...
{\begin{myIndent}\hTwo
   ...
   \begin{enumerate}
      ...
      \item[(B)] If we define $g: W \longrightarrow \mathbb{R}^n$ such that each $\mVec{y} \in W$ is mapped to the unique\\ $\mVec{x}$ described above, then $f(g(\mVec{y}), \mVec{y}) = \mVec{0}$ for all $\mVec{y} \in W$,\hspace{0.3em} $g(\mVec{b}) = \mVec{a}$, and\\ $g \in \mathscr{C}^1(W, \mathbb{R}^n)$ with $g^\prime(\mVec{b}) = -\mMat{A}_x^{-1}\mMat{A}_y$.\retTwo
   \end{enumerate}
...
{\begin{myIndent}\hTwo
   ...
   \begin{itemize}
      ...
      {\begin{myIndent}\hThree
         ...
         Clearly, $\mMat{P}$ is a linear operator with $\rangeSp{\mMat{P}} = \mySpan \{x_1, \ldots, x_k\} = X_1$.\\ Also, $\mVec{y} \in X_1 \Longrightarrow \mVec{y} = \mMat{P}\mVec{y}$. Thus, $\mMat{P}$ is  a projection since\\ $\mMat{P}\mVec{x} = \mMat{P}(\mMat{P}\mVec{x})$ for all $\mVec{x} \in X$.
      \end{myIndent}}
   \end{itemize}
\end{myIndent}}

\color{red}\mySepOne\\ [-24pt]
\begin{myIndent}
	Note from 7/26/2026: Rudin's presentation of this theorem is really bad. For example, he never defines what it means for $\varphi$ to be differentiable on $\mMat{A}(V)$ (since $\mMat{A}(V)$ isn't an open set of $\mathbb{R}^m$). Furthermore, his notation is so unclear that even with over two years more experience I still can't understand this theorem when I read it. Anyways, \textit{I have not ever needed this theorem a single time before this date}. Also I just proved this theorem again but with much better notation on page 1062 of my math journal. And for a simpler subcase that will give you much of the intuition you might want, you can look at page 926 of my journal. Anyways I'm only not deleting this proof from my notes because I don't want to ruin page numbering.\retTwo
\end{myIndent}

\hOne With this background, we now present the following theorem:\\ [-9pt]
{\begin{myIndent}\hTwo
   ...
   {\begin{myIndent}\hThree
      Proof:\\
      If $r = 0$, then letting $R > 0$ be such that $B_r(\mVec{a}) \subseteq E$, we know that $f$\\ is constant on $B_R(\mVec{a})$. Thus, set $V = U = B_R(\mVec{a})$, $\myHS H(\mVec{x}) = \mVec{x}$, and\\ [-1pt] $\varphi(\mVec{0}) = F(\mVec{a})$. Clearly, $H \in \mathscr{C}^1(V, U)$ and is its own inverse.  Also,\newpage while it doesn't make sense to say that $\varphi \in \mathscr{C}^1(\mMat{A}(V), Y_2)$ since\\ [-2pt] $\mMat{A}(V) = \{\mVec{0}\}$ has no limit points, we do have that since $\mMat{A}(V) = Y_1$,\\ trivially $\mMat{A}(V)$ is open relative to $Y_1$. And since $Y_2 = \mathbb{R}^m$, we know\\ [-1pt] that $\phi(\mVec{0}) \in Y_2$. Finally, we have for all $\mVec{x} \in V$ that:
      ...
      In turn, we can then replace $\mVec{x}$ with $H(\mVec{x})$ and say that for all $\mVec{x} \in V$,\\ $\mMat{P}F(H(\mVec{x})) = \mMat{A}(G(H(\mVec{x}))) = \mMat{A}\mVec{x}$.\newpage
      
      Then, having defined $\Psi(\mVec{x}) = F(H(\mVec{x})) - \mMat{A}\mVec{x}$, note that for all $\mVec{x} \in V$,\\ $\mMat{P} \Psi(\mVec{x}) = \mMat{P}F(H(\mVec{x})) - \mMat{A}\mVec{x} = \mVec{0}$. Also, $\Psi^\prime(\mVec{x}) = F^\prime(H(\mVec{x}))H^\prime(\mVec{x}) - \mMat{A}$\\ is continuous. So, $\Psi \in \mathscr{C}^1(V, P_2)$.\\ [6pt]

      Finally, we just need to show the existence of a $\mathscr{C}^1$-mapping $\varphi$ from $\mMat{A}(V)$\\ to $Y_2$ satisfying that $\varphi(\mMat{A}\mVec{x}) = \Psi(\mVec{x})$ for all $\mVec{x} \in V$. To start, we first show\\ that if $\mVecAst{x}_1 \in V$, $\hspace{0.3em}\mVecAst{x}_2 \in V$, and $\mMat{A}\mVecAst{x}_1 = \mMat{A}\mVecAst{x}_2$, then $\Psi(\mVecAst{x}_1) = \Psi(\mVecAst{x}_2)$.
      {\begin{myIndent}\hFour
         ...
         Thus, $\rank{\Phi^\prime(\mVec{x})} = \rank{F^\prime(H(\mVec{x}))} = r$  for all $\mVec{x} \in V$.\retTwo
      ...
      Therefore, we can umambiguously define $\varphi: V(A) \longrightarrow Y_2$ by the\\ expression $\varphi(\mMat{A}\mVec{x}) = \Psi(\mVec{x})$. Furthermore, we can show that $\varphi$ is $\mathscr{C}^1$.
      {\begin{myIndent}\hFour
         ...
         Then because $\mVec{y} = \mMat{A}(\mVecAst{x}_0 + \mMat{S}(\mVec{y} - \mVecAst{y}_0))$ for all $\mVec{y} \in W$, we have that\\ $\varphi(\mVec{y}) = \Psi(\mVecAst{x}_0 - \mMat{S}(\mVecAst{y}_0 - \mVec{y}))$. Hence, $\varphi$ is continuously\\ differentiable with respect to $\mVec{y} \in W$.
			\begin{myIndent}\color{red}
				Note from 7/26/2026: RUDIN NEVER DEFINED WHAT THIS MEANS!!! WHAT DO YOU MEAN!!\retTwo
			\end{myIndent}
			
			And since $W$ and $\mVecAst{y}_0$ was arbitrary, we have that $\varphi$ is continuously differentiable on $\mMat{A}(V)$. $\blacksquare$\retTwo
      \end{myIndent}}
   \end{myIndent}}
   % \teachComment
...
\hTwo To finish off this chapter (I'm skipping the segment on determinants), here is a\\ theorem about differentiation of integrals.\\ [-6pt]
...
{\begin{myIndent}\exTwo
   ...
   In either case, by combining the three observations above we can say\\ for any permutation $\sigma \in S_k$ that $D_{i_1,i_2,\ldots,i_k}f = D_{i_{\sigma(1)}i_{\sigma(2)},\ldots,i_{\sigma(k)}}f$.
\end{myIndent}}
\mySepTwo

\hOne\markLecture{5/2/2024}

Everything in my notes after this point would be better reviewed from my math 240a notes. Anyways a note on notation:\\ [-20pt]
\begin{itemize}
...
{\begin{myIndent}\hTwo
   And since $\bigcap\limits_{n=1}^\infty A_n = A_1 - \bigcup\limits_{n=1}^\infty (A_1 - A_n)$, we know $\bigcap\limits_{n=1}^\infty A_n \in \mathcal{R}$ if $\mathcal{R}$ is a $\sigma$-ring.\retTwo
\end{myIndent}}
...
\begin{enumerate}
   ...
   {\begin{myIndent}\exTwo
      ...
      {\centering$A \subseteq \bigcup\limits_{n=1}^\infty A_n$ and $\mu^*(A) \leq \sum\limits_{n=1}^\infty m(A_n) \leq \mu^*(A) + \varepsilon$.\retTwo\par}
