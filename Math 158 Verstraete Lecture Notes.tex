...
The \udefine{chromatic number} $\chi(G)$ is the minimum $k$ for which $G$ has a proper $k$-coloring. \retTwo
...
{\begin{myIndent} \hTwo
   \uuline{Erd\H{o}s-Simonovitz-Stone theorem}: If $F$ is any graph of chromatic number $r+1$, then:
...
\markLecture{3/5/2024}

{\begin{myIndent} \hTwo
   \uuline{Even Cycle Theorem}:\\
   For some constant $c_k$ (which we prove is at most $8k$): $\exNums{n, C_{2k}} \leq c_kn^{1 + \frac{1}{k}}$. \retTwo

   \begin{myIndent}
      \color{BrickRed}%
      The proof for this is rather long. So because of time crunch, it has been moved to the end of the notes on page <put a number>.
   \end{myIndent}
\end{myIndent}}

\mySepTwo

\markLecture{3/7/2024}

The \udefine{Ramsey number} $r(s, t)$ for $s, t \in \mathbb{Z}^+$ is the minimum $N$ such that whenever the edges of $K_N$ are colored red and blue, either a red $K_s$ occurs or a blue $K_t$ occurs. \retTwo

Equivalently, $r(s, t)$ is the minimum $N$ such that every $K_s$-free graph on $N$ vertices contains an \udefine{independent set} of size $t$.\\
{\begin{myDindent} \myComment
      To quote from homework 3 of the class:\\
      An \textbf{independent set} in a graph $G$ is a set $X \subseteq V(G)$ such that $e(X) = 0$, and the \textbf{independence number} $\alpha(G)$ is the largest size of an independent set in $G$.\\
\end{myDindent}}

\newpage

A $K_s$-free graph with $r(s, t) - 1$ vertices and no independent set of size $t$ is called\\ a \udefine{Ramsey graph}, and a coloring of $K_n$ with $n = r(s,t) - 1$ vertices and no\\ monochromatic $K_s$ or $K_t$ is called a \udefine{Ramsey coloring}.\retTwo
...
{\begin{myTindent} \myComment
   \begin{myClosureOne}{4.5}
      Here's a hopefully obvious observation:\\
      If all red-blue colorings of $K_N$ contain a forbidden graph, then\newline all red-blue colorings of $K_{N+1}$ will contain the forbidden graph\newline because $K_N$ is a subgraph of $K_{N+1}$.\\
   \end{myClosureOne}
\end{myTindent}}
\retTwo
{\begin{myIndent} \hTwo
   \uuline{Erd\H{o}s-Szekeres Theorem}:\\[2pt]
   For $s \geq 2$ and $t \geq 2$, $\myHS r(s, t) \leq r(s-1, t) + r(s, t-1)$. In particular:
   \[r(s,t) \leq \binom{s+t-2}{s-1}\]
   
   {\begin{myIndent}\hThree
      Proof:\\
      $r(1, t) = r(s, 1) = 1$ trivially. Also trivially, if $s = 2$, then $r(s, t) = t$.\\ Meanwhile if $t = 2$, then $r(s, t) = s$. Indeed, both of these satisfy the\\ inequalities of the theorem. \retTwo

      \newpage

      Now we proceed by induction, assuming $s \geq 3$, $t \geq 3$, and that the theorem is true when $s$ or $t$ is smaller.\retTwo

      {\begin{myIndent} \hFour
         Let $N = r(s, t)$ and suppose we are given a red-blue coloring\\ of $E(K_N)$. Picking $v \in V(K_N)$, we know $d(v) = N - 1$. Then if\\
         $N \geq r(s-1, t) + r(s, t-1) + 1$, we get at least either $r(s-1, t)$\\ red edges on $v$ or $r(s, t-1)$ blue edges on $v$. \retTwo

         If there are $r(s-1, t)$ red edges on $v$, then the set $N_{\text{red}}(v)$ of\\ vertices joined to $v$ by a red edge contains either a red $K_{s-1}$\\ or a blue $K_t$ by induction. This is because $|N_{\text{red}}(v)| \geq r(s-1, t)$.\\
         Now if there is a blue $K_t$ contained in the vertices of $N_\text{red}(v)$, we're\\ done. Otherwise, the red $K_{s-1}$ contained in the vertices of $N_\text{red}(v)$\\ combined with $v$ itself form $K_s$. \retTwo

         As for if there are $r(s, t-1)$ blue edges on $v$, then we can do similar\\ reasoning to show that $K_n$ contains either a red $K_s$ or blue $K_t$. \retTwo
      \end{myIndent}}

      So $r(s, t) \leq r(s-1, t) + r(s, t-1)$ for $s \geq 2$ and $t \geq 2$. Now let's get the non-recursively defined inequality.\retTwo

      We already said that $r(2, t), r(s, 2) \leq \binom{s+t-2}{s-1}$. Now once again proceed by\\ induction, assuming $s \geq 3$, $t \geq 3$, and that the theorem is true when $s$ or $t$\\ is smaller. \retTwo

      {\begin{myIndent} \hFour
         \begin{tabular}{l}
            $r(s, t) \leq r(s-1, t) + r(s, t-1)$ \\ [3pt]
            $\phantom{r(s, t)} \leq {\displaystyle \binom{s+t-3}{s-2}} + {\displaystyle \binom{s+t-3}{s-1}}$ 
         \end{tabular}
         \retTwo

         Now use \uuline{Pascal's Triangle Identity}: ${\displaystyle \binom{a}{b} = \binom{a-1}{b} + \binom{a-1}{b-1}}$.\retTwo
         
         \begin{myIndent}
            {\myComment\begin{myClosureOne}{4}
               Here is a quick algebraic proof of this identity:\\ [3pt]
               \begin{tabular}{l}
                  $\binom{a-1}{b} + \binom{a-1}{b-1} = (a-1)!\left(\frac{1}{b!(a-b-1)!} + \frac{1}{(b-1)!(a-b)!}\right)$\\ [5pt]
                  $\phantom{\binom{a-1}{b} + \binom{a-1}{b-1}} = (a-1)!\left(\frac{(a-b)}{b!(a-b)!} + \frac{b}{b!(a-b)!}\right)$\\ [5pt]
                  $\phantom{\binom{a-1}{b} + \binom{a-1}{b-1}} = \frac{a!}{b!(a-b)!} = \binom{a}{b}$
               \end{tabular} \\
            \end{myClosureOne}}
         \end{myIndent}
         \retTwo

         Letting $a = s + t - 2$ and $b = s - 1$, we can say that:
...
{\begin{myIndent} \hTwo
   \uuline{Theorem (Erd\H{o}s, 1947)}: $r(t, t) \geq \sqrt{2}^t$ for $t \geq 3$. \retTwo
   
   {\begin{myIndent} \hThree
      Proof:\\
      Let $N = \lfloor \sqrt{2}^t \rfloor$. Color $E(K_N)$ randomly such that each edge independently has $\frac{1}{2}$ probability to be red rather than blue.\\

      The average number of red $K_t$ is: $\mathbb{E}(\text{\# of red } K_t) = {\displaystyle \binom{N}{t}\left(\dfrac{1}{2}\right)^{
      \begin{pmatrix}
         t \\ 2
      \end{pmatrix}}}$\\
      ...
      {\begin{myIndent} \hFour
         Explanation:\\
         There are $\binom{N}{t}$ possible groupings of $t$ vertices in $K_N$. Then $\left(\frac{1}{2}\right)^{\left(
         \begin{smallmatrix}
            t \\ 2
         \end{smallmatrix}\right)}$ is the probability that every edge in a grouping of $t$ vertices will be colored red.
      \end{myIndent}}

      \newpage

      Similarly, $\mathbb{E}(\text{\# of blue } K_t) = {\displaystyle \binom{N}{t}\left(\dfrac{1}{2}\right)^{
         \begin{pmatrix}
            t \\ 2
         \end{pmatrix}}}$\retTwo

      Thus the average number of red or blue $K_t$ is:

      {\centering ${\displaystyle 2\binom{N}{t}\left(\frac{1}{2}\right)}^{
      \begin{pmatrix}
         t \\ 2
      \end{pmatrix}} \leq 2\dfrac{N^t}{t!}\cdot2^{\frac{-t(t-1)}{2}} \leq 2\dfrac{(\sqrt{2})^{t^2}}{t!}\cdot 2^{\frac{-t^2}{2} + \frac{t}{2}} = \dfrac{2}{t!}\cdot 2^{\frac{t}{2}}$ \retTwo\par}

      When $t \geq 3, \myHS \dfrac{2}{t!}\cdot 2^{\frac{t}{2}} < 1$ (you can visually check that on Desmos). This tells us that there must be a red-blue coloring of $K_N$ with no red $K_t$ or blue $K_t$.\retTwo

      {\begin{myTindent} \teachComment
         Side note: this was the first proof ever done using\\ probabilistic combinatorics.\retTwo
      \end{myTindent}}
   \end{myIndent}}

   Using the Erd\H{o}s-Szekeres theorem, we can also get the following upper bound:

   {\hThree\begin{myIndent}
      $r(t,t) \leq {\displaystyle \binom{2t-2}{t-1}} \leq \dfrac{(2t-2)!}{(t-1)!(t-1)!} \leq \dfrac{(2(t-1))^{2t-2}}{\left((t-1)^{t-1}\right)^2}\hspace{0.75em}$ {\myComment(because $n! \leq n^n$)}\\ [5pt]
      $\hphantom{r(t,t) \leq {\displaystyle \binom{2t-2}{t-1}} \leq \dfrac{(2t-2)!}{(t-1)!(t-1)!}} \leq \dfrac{2^{2t-2}(t-1)^{2t-2}}{(t-1)^{2t-2}}$\\ [5pt]
      $\hphantom{r(t,t) \leq {\displaystyle \binom{2t-2}{t-1}} \leq \dfrac{(2t-2)!}{(t-1)!(t-1)!}} \leq 2^{2t-2} = 4^{t-1}$\retTwo
   \end{myIndent}}

   Thus $r(t, t)$ grows exponentially with $t$.\retTwo
\end{myIndent}}

\mySepTwo

{\huge \color{Black} \textbf{Discussion Section Additional Notes} \newline}

Generalization of Ramsey numbers:
\begin{myIndent}
   Let $H$ and $F$ be graphs. Then $r(H, F)$ is the minimum $N$ such that any\\ red-blue coloring of $E(K_N)$ will have either a red $H$ or blue $F$. Note\\ that $r(s, t) = r(K_s, K_t)$.
   \retTwo
\end{myIndent}
\retTwo

\hOne An equivalent way of thinking about Ramsey numbers is through the complements\\ [-3pt] of graphs. Let $G$ be an $n$-vertex graph. Then the \udefine{complement} of $G$ is $\overbar{G}$ where $\overbar{G}$ has the same vertices as $G$ and all the edges that $G$ doesn't have. \retTwo
...
{\begin{myIndent} \hTwo
   \uuline{Schur's Theorem}: If $n \geq 2$ and $p > \lfloor e\cdot n! \rfloor + 1$ is prime, then $x^n + y^n \myequiv{p} z^n$ has a\\ solution modulo $p$.\retTwo

   \hThree Proof:\\
   Start by defining $r_k(3)$ to be the minimum $N$ such that if we color $E(K_N)$ with $k$ colors, we will get a monochromatic triangle.\\
   
   {\begin{center} \hFour
      \begin{tabular}{ c c c c}
         $r_2(3) = r(3, 3) = 6$ & \quad & \quad & $r_3(3) = 17$ (this is the highest exact value known)
      \end{tabular}
   \end{center}}

   \newpage

   \uuline{Theorem}: For $k \geq 2, \myHS r_k(3) \leq \lfloor e\cdot k! \rfloor + 1$\\ [-11pt]
   {\begin{myIndent} \hFour
      Proof:\\
      Let us $k$-color $E(K_N)$ and proceed by induction on $k$.\retTwo

      Assume that $K_N$ has no monochromatic triangles. Then pick any $u \in V(K_N)$. By the pigeonhole principle, there must be some color with at least $\frac{N-1}{k}$ edges touching $u$. Letting $U$ be the set of neighbors of $u$ in that color, we have that $|U| \geq \frac{N-1}{k}$.\retTwo

      Now if for any two vertices $x, y \in U$, we had that $\{x, y\}$ is colored the same as\\ $\{x, u\}$ and $\{y, u\}$, then we would have a monochromatic triangle between $x$, $y$,\\ and $u$. So the edges in $U$ must have at most $k - 1$ colors. Since, there must not be\\ a monochromatic triangle in any of the other colors, we know $|U| 
      < r_{k-1}(3)$.\retTwo

      Thus, we know that $\frac{N-1}{k} < r_{k-1}(3) $. Or in other words, $\frac{N-1}{k} \leq r_{k-1}(3) - 1$. Rewriting this, we get that $N \leq kr_{k-1}(3) - k + 1$. So: \\ [-12pt]

      {\center $r_k(3) - 1 \leq k(r_{k-1}(3) - 1) + 1$. \retTwo\par}
...
{\centering\raisebox{0em}{\tikz[scale=0.6,inner sep=4pt]{
   ...
   \node at (2.95,-2.6) {{\exTwo$2$}};
   \node at (6.7,3.65) {{\exTwo$1$}};
   \node at (9.2,-1.1) {{\exTwo$4$}};
   \node at (6.7,-3.65) {{\exTwo$2$}};
   \node at (9.2,1.1) {{\exTwo$4$}};
   \node at (13.3,1.8) {{\exTwo$3$}};
   \node at (13.3,-1.8) {{\exTwo$2$}};
}}\par}
...
A \udefine{capacity} function is a map $c: E(G) \rightarrow \mathbb{R}^{\geq 0}$ such that if $f: E(G) \rightarrow \mathbb{R}$ is a flow,\\ then $f(v, w) \leq c(v, w)$ for all $(v, w) \in E(G)$.\retTwo

{\center\teachComment
\begin{myClosureOne}{5}
   Note that if you want to have more than one source, you can treat several sources as all being connected to one supersource. A similar trick also works if you want multiple sinks.\\ [-2pt]
\end{myClosureOne}\par}

\retTwo

\markLecture{3/14/2024}

Let $G$ be a digraph and $f$ a flow with a source $s$ and sink $t$. If we have sets $S$ and\\ $T$ that partition $V(G)$ such that $s \in S$ and $t \in T$, then we can define the \udefine{$s$-$t$ cut}:\\ $(S, T)$, to be the spanning subgraph of $G$ containing all the edges going out of $S$\\ into $T$.\retTwo

{\begin{myIndent} \hTwo
   \uuline{Theorem}: For any flow $f$ and $s$-$t$ cut $(S, T)$, $\hspace{0.5em} v(f) = \hspace{-1em}\sum\limits_{
   \begin{smallmatrix}
      (x, y) \in E(G) \\
      x \in S \\
      y \in T
   \end{smallmatrix}}\hspace{-1.1em}f(x, y)\hspace{0.3em} - \hspace{-1em}\sum\limits_{
      \begin{smallmatrix}
         ...
         x \in S \\
         y \in T
      \end{smallmatrix}}\hspace{-1.1em}f(y, x)$.

   \retTwo
   \begin{myIndent}
      \hThree Proof:\\
      By Kirchoff's law, for all $x \in S \setminus \{s, t\}$, $\myHS\sum\limits_{y \in N^+(x)}\hspace{-0.65em}f(x, y) = \hspace{-0.8em}\sum\limits_{y \in N^-(x)}\hspace{-0.65em}f(y, x)$.\\ [13pt]

      Thus, $\sum\limits_{x \in S\setminus \{s\}} \hspace{-0.3em} \left( \hspace{0.1em}\sum\limits_{y \in N^+(x)} \hspace{-0.7em} f(x, y) \right) = \sum\limits_{x \in S\setminus \{s\}} \hspace{-0.3em} \left( \hspace{0.1em}\sum\limits_{y \in N^-(x)} \hspace{-0.7em} f(y, x) \right)$.

      \newpage
      
      Meanwhile, by the definition of $v(f)$:$\hspace{-0.2em}\sum\limits_{y \in N^+(s)}\hspace{-0.65em}f(s, y) = \hspace{-0.8em}\sum\limits_{y \in N^-(s)}\hspace{-0.65em}f(y, s) + v(f)$.\\ [13pt]
   
      So, $\sum\limits_{x \in S} \hspace{-0.3em} \left( \hspace{0.1em}\sum\limits_{y \in N^+(x)} \hspace{-0.7em} f(x, y) \right) = \sum\limits_{x \in S} \hspace{-0.3em} \left( \hspace{0.1em}\sum\limits_{y \in N^-(x)} \hspace{-0.7em} f(y, x) \right) + v(f)$.\\ [13pt]
   
      Now for each $x \in S$ and $y \in N^+(x)$, if we also have that $y \in S$, then $f(x, y)$\\ will be added on both sides of the equation. So we can cancel out those edges, leaving us with the equation of the theorem. \retTwo
   \end{myIndent}

   \uuline{Corollary}: If $t$ is the sink of a flow $f$, then $-v(f) = \hspace{-0.7em}\sum\limits_{v \in N^+(t)}\hspace{-0.7em}f(t, v)\hspace{0.2em} - \hspace{-0.7em}\sum\limits_{v \in N^-(t)}\hspace{-0.7em}f(v, t)$.

   {\begin{myIndent} \hThree
      Proof:\\
      Take $S = V(G) \setminus \{t\}$ and $T = \{t\}$. Then by the above theorem,\\ $\hspace{-0.7em}\sum\limits_{v \in N^-(t)}\hspace{-0.7em}f(v, t)\hspace{0.2em} = \hspace{-0.7em}\sum\limits_{v \in N^+(t)}\hspace{-0.7em}f(t, v) + v(f)$.\retTwo
   \end{myIndent}}
\end{myIndent}}

Given a capacity function $c$ and $s$-$t$ cut $(S, T)$, we define the \udefine{capacity of an $s$-$t$ cut}\\ as: 
   
{\centering $c(S, T) = \hspace{-0.5em}\sum\limits_{e \in (S, T)}\hspace{-0.5em}c(v, w)$.\retTwo\par}

{\begin{myIndent} \hTwo
   \uuline{Max Flow Min Cut Theorem}: The maximum value of any flow equals the minimum capacity of any $s$-$t$ cut.\retTwo

   {\begin{myIndent} \hThree
      Proof:\\
      Let $c$ be the capacity function for a digraph $G$ with a source $s$ and sink $t$.\\ For any flow $f$ and $s$-$t$ cut $(S, T)$, we must have that $v(f) \leq c(S, T)$.\\ [-14pt]
      {\begin{myIndent} \hFour
         To see why, first note that:\\
         $v(f) = \hspace{-1em}\sum\limits_{
         \begin{smallmatrix}
            (x, y) \in E(G) \\
            x \in S \\
            y \in T
         \end{smallmatrix}}\hspace{-1.1em}f(x, y)\hspace{0.3em} - \hspace{-1em}\sum\limits_{
            \begin{smallmatrix}
               ...
               x \in S \\
               y \in T
            \end{smallmatrix}}\hspace{-1.1em}f(y, x) \leq 
         \hspace{-1em}\sum\limits_{
            \begin{smallmatrix}
               (x, y) \in E(G) \\
               x \in S \\
               y \in T
            \end{smallmatrix}}\hspace{-1.1em}f(x, y)\hspace{0.3em}
         \leq 
         \hspace{-1em}\sum\limits_{
            \begin{smallmatrix}
               (x, y) \in E(G) \\
               x \in S \\
               y \in T
            \end{smallmatrix}}\hspace{-1.1em}c(x, y)\hspace{0.3em}$\\ [13pt]
      ...
      {\begin{myTindent}\begin{myDindent} \teachComment
         \begin{myClosureOne}{3}
            Note that this proof can also be interpretted\\ as an iterative algorithm for finding a\\ maximum flow and minimum cut.\\ [-2pt]
         \end{myClosureOne}
      \end{myDindent}\end{myTindent}}
   \end{myIndent}}
\end{myIndent}}
...
To conclude the class, here is another proof of Hall's theorem using the max flow\\ min cut theorem which notably can also be interpretted as an algorithm for finding\\ a maximum matching on a bipartite graph.\retTwo

{\begin{myIndent} \hTwo
   \uuline{Hall's Theorem}: Let $G$ be a bipartite graph with parts $A$ and $B$. Then $G$ has a \\matching saturating $A$ if and only if for every set $X \subseteq A,\myHS \lvert N_G(X) \rvert \geq \lvert X \rvert$.

   \begin{myIndent}\hThree
      Proof:\\
      ($\Longrightarrow$) This direction is trivial to prove. So we don't care about it here.\retTwo

      ($\Longleftarrow$) Assume $G$ is a bipartite graph with parts $A$ and $B$ satisfying Hall's\\ condition. Then orient all the the edges of $G$ to be directed from $A$ to $B$\\ and add a vertex $s$ adjacent to all of $A$ and a vertex $t$ adjacent to all of $B$.

      \newpage

      Now that $G$ is a digraph, define a capacity function $c$ assigning each edge\\ of $G$ a capacity of $1$, and make $s$ and $t$ the source and sink respectively of $G$.\\ Having done this, if $f$ is a max flow of $G$, then $v(f)$ equals the size of an\\ optimal matching.\retTwo
      
      
      
      Define $(S, T)$ to be any $s$-$t$ cut of $G$. Then $c(S, T) = |E((S, T))|$ and\\ $E((S, T))$ consists of all edges going from $s$ to $(A \setminus S)$, $(A \cap S)$ to $(B \setminus S)$,\\ and $(B \cap S)$ to $t$.\retTwo
      
      
      edges going from $(A \setminus T)$ to $(A \setminus S)$. \retTwo



      Let $X = (A \setminus S)$ and $Y = (B \cap S)$. Sin $c(S, T) = |E((S, T))|$ and\\ $|E((S, T))|$ consists of edges going from $s$ to $(A \setminus S)$, and $(A \cap S)$ to $(B \setminus S)$.
      
      we know that $c(S, T) \geq |A \setminus S| + |B \setminus S|$.
