...
{\begin{myIndent} \hTwo
   ...
   {\begin{myIndent} \hFour
      ...
      Assume that $K_N$ has no monocromatic triangles. Then pick any $u \in V(K_N)$. By the pigeonhole principle, there must be some color with at least $\frac{N-1}{k}$ edges touching $u$. Letting $U$ be the set of neighbors of $u$ in that color, we have that $|U| \geq \frac{N-1}{k}$.\retTwo
...
{\begin{myIndent} \hTwo
   \uuline{Theorem}: For any flow $f$ and $s$-$t$ cut $(S, T)$, $\quad v(f) = \hspace{-1em}\sum\limits_{
   \begin{smallmatrix}
      (x, y) \in E(G) \\
      x \in S \\
      y \in T
...
Given an $s$-$t$ cut $(S, T)$ and capacity function $c$, the \udefine{capacity of an $s$-$t$ cut} is defined\\ as: 
...
{\begin{myIndent} \hTwo
   ...
   {\begin{myIndent} \hThree
      Proof:\\
      For any flow $f$ and $s$-$t$ cut $(S, T)$, we must have that $v(f) \leq c(S, T)$.
      {\begin{myIndent} \hFour
      ...
      Let $f$ be a maximum flow in $G$. Then define a set $S \subset V(G)$ as follows:
      \begin{itemize}
         \item Start by putting $s$ into $S$.
         \item Then repeatedly until there are no more $y$ such that $(x, y) \in E(G)$\\ where $x \in S$ and $f(x, y) < c(x, y)$, put $y$ into $S$.\retTwo
      \end{itemize}
