...
\newpage
\exOne
Here's an example of flow in a digraph:\\

{\centering\raisebox{0em}{\tikz[scale=0.7,inner sep=4pt]{
...
{\center\teachComment
\begin{myClosureOne}{5}
   Note that if you want to have more than one source, you can treat several sources as all being connected to one supersource. A similar trick also works if you want multiple sinks.\\
...
Question: In a digraph with capacities and a source and sink, what is the maximum value of a flow that we can have?

{\begin{myIndent} \hTwo
   Let $(S, T)$ be a cut of $G$. Note that we are distinguishing between $(S, T)$ and $(T, S)$.\retTwo

   \uuline{Theorem}: For any flow $f$ and cut $(S, T)$, we have that:
   ...
   \end{smallmatrix}}\hspace{-1.1em}f(x, y)\hspace{0.3em} - \hspace{-1em}\sum\limits_{
      \begin{smallmatrix}
         ...
         x \in S \\
         y \in T
      \end{smallmatrix}}\hspace{-1.1em}f(y, x)$\par}
   
   \newpage

   The \udefine{capacity of a cut} is defined as: 
   
   {\center $c(S, T) = \hspace{-0.2em}\sum\limits_{
   \begin{smallmatrix}
      v \in S \\
      w \in T
   \end{smallmatrix}}\hspace{-0.1em}c(v, w)$.\par}\retTwo

   \uuline{Max Flow Min Cut Theorem}: The maximum value of any flow equals the minimum capacity of any cut.\retTwo

   {\color{BrickRed} Finish this after you're done.}
