...
A \udefine{Sidon set} in an abelian group $\Gamma$ (with the group operation $+$) is a set $A \subseteq \Gamma$ such that if $a+b = a^\prime + b^\prime$ with $a, b, a^\prime, b^\prime \in A$, then $\{a, b\} = \{c, d\}$. \retTwo
...
{\begin{myIndent} \hTwo
   Here's an obvious upper bound to $|A|$:\\ $\binom{|A|}{2} \leq |\Gamma|$. This is because the set of the all pairs of $a$ and $b$ must be mapped to different elements of $\Gamma$ by the operation $+$.

   \newpage

   \uuline{Proposition}: The maximum size of a Sidon set in the abelian group $\Gamma$ of order $n$ is at most $x$ where $x(x-1) \leq n - 1$.
