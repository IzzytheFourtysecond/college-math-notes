...
\begin{document}
   ...
   \begin{myIndent}\begin{myIndent}
      {\teachComment
         In other words, this theorem is stating that $\mathbb{R}$ exists and is unique. Unfortunately, the proof for this is very long and so won't be covered in lecture.
      }
   \end{myIndent}\end{myIndent}
   ...
   {\begin{myIndent} \hTwo
      ...
      {\begin{myIndent}\hThree
         Proof:\\
         Pick $\varepsilon > 0$. Then, we know there exists $M$ such that for all $n > M$\\ $0 \leq s_n \leq \varepsilon$. However, $0 \leq x_n \leq s_n$. So for all $n > M$, we have that\\ $0 \leq x_n \leq s_n <\varepsilon$. Hence, $x_n$ converges to $0$.\retTwo \retTwo
      \end{myIndent}}
   ...
   Let $f: (a, b) \rightarrow \mathbb{R}$. If $a \leq x < b$, we define the \udefine{right-hand limit} of $f$ at $x$ and denote this $f(x+)$ or $\lim\limits_{t\rightarrow x^+}f(t)$ to be $\lim\limits_{t\rightarrow x}g(t)$ where $g$ is the restriction of $f$ to $(x, b)$.\retTwo

   Similarly, if $a < x \leq b$, we define the \udefine{left-hand limit} of $f$ at $x$ and denote this $f(x-)$\\ or $\lim\limits_{t\rightarrow x^-}f(t)$ to be $\lim\limits_{t\rightarrow x}h(t)$ where $h$ is the restriction of $f$ to $(a, x)$.\retTwo
   
   {\begin{myIndent} \exOne
      Observation: $\lim\limits_{t\rightarrow x}$ exists if and only if both $f(x-)$ and $f(x+)$ exist and are equal to each other. In this case, all three are equal to each other. \retTwo
   \end{myIndent}}

   If $f$ is discontinuous at $x$ and both $f(x-)$ and $f(x+)$ are defined, then we say that $x$ is a \udefine{discontinuity of the first kind} or a \udefine{simple discontinuity}. Otherwise, we say $x$ is a \udefine{discontinuity of the second kind}.

   \mySepTwo

   \markLecture{3/13/2024}

   A function $f: (a, b) \rightarrow \mathbb{R}$ is \udefine{monotone increasing} if $f(x_1) \leq f(x_2)$ whenever\\ $x_1 \leq x_2$. Similarly, $f$ is \udefine{monotone decreasing} if $f(x_1) \geq f(x_2)$ whenever $x_1 \leq x_2$.
