...
\begin{document}
   ...
   A sequence $(p_n)_{n \in \mathbb{N}}$ in a metric space $X$ \udefine{converges} if there is $p \in \mathbb{X}$ such that:
   \[\forall \varepsilon > 0, \hspace{0.5em} \exists N \in \mathbb{N} \suchthat \forall n \geq N, \hspace{0.5em} p_n \in B_\varepsilon(p)\]

   When this occurs, we say that $(p_n)$ converges to $p$ or that $p$ is a limit of $(p_n)$, and we write this as: $p_n \rightarrow p$ or $\lim_{n\rightarrow\infty}{p_n} = p$.
