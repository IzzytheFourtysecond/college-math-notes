...
\begin{enumerate}
	...
	\begin{myIndent}\exTwoP
		The simplest case is when $f$ and $g$ are both nonnegative real-valued functions. Then we can find increasing sequences of simple functions $(\phi_n)$ and $(\varphi_n)$ such that $\phi_n \rightarrow f$ and $\varphi_n \rightarrow g$.\retTwo

		Now for all $n$, define $\psi_n(x, y) = \phi_n(x)\varphi_n(y)$. It's clear that $\psi_n$ can only equal a finite amount of values (at most the number of values $\phi_n$ can take on times the number of values $\varphi_n$ can take on). Also by part (a), we know that $\psi_n$ is measurable for all $n$.\newpage
		
		Thirdly, note that $(\psi_n)$ is an increasing sequence of functions converging\\ pointwise to $h$. \retTwo
