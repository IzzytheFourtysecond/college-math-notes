...
\begin{enumerate}
   ...
   \begin{myIndent}\exTwoP
      ...
      Next for all $j, i \in \mathbb{N}$, pick $A_j^{(i)} \in \mathcal{A}_\sigma$ satisfying that $E \cap C_i \subseteq A_j^{(i)}$ and\\ $\mu^*(E) \leq \mu^*(A_j^{(i)}) \leq \mu^*(E) + \sfrac{\varepsilon}{j2^{i}}$. Since $\mu^*(E \cap C_i)$ is finite, we can use the same reasoning as in part (b) to say that $\mu^*(A_j^{(i)} - (E \cap C_i)) < \sfrac{\varepsilon}{j2^i}$.\retTwo

      Importantly, $A_j^{(i)} 

      Finally, note that $\bigcup\limits_{i\in \mathbb{N}}A_{j}^{(i)} \in \mathcal{A}_{\sigma}$ and $\bigcup\limits_{i\in\mathbb{N}}(A_j^{(i)} - (E \cap C_i))$
   \end{myIndent}
