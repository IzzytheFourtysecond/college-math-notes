...
\begin{myIndent}\hTwo
   \blab{Proposition:} Let $F: \mathbb{R} \longrightarrow \mathbb{R}$ be a monotonically increasing and right continuous function (meaning $\lim\limits_{t\rightarrow x^+}F(t) = F(x)$ for all $x \in \mathbb{R}$). Also, define $F(-\infty)$ and $F(\infty)$\\ [-6pt]\phantom{aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa} respecting the monotonicity of $F$.\retTwo If $(a_j, b_j]$ for $j = 1,\ldots, n$ are disjoint intervals in $H$, define:

   {\centering $\mu_0(\bigcup\limits_{j=1}^n (a_j, b_j]) = \sum\limits_{j=1}^n F(b_j) - F(a_j)$ \\[-2pt]\par}

   Also let $\mu_0(\emptyset) = 0$. Then this is a premeasure.
   ...
   \begin{myIndent}\hThree
      ...
      \begin{enumerate}
         ...
         \begin{myIndent}\hFour
            Since $\bigcup\limits_{j\in\mathbb{N}}I_j \in \mathcal{A}$, we know it is equal to a finite union of disjoint intervals of $H$. By considering those intervals separately, we can thus assume without\\ [6pt] loss of generality that $\bigcup\limits_{j \in \mathbb{N}}I_j = (a, b]$.\retTwo

            Now it's obvious from the construction of $\mu_0$ that $\mu_0$ is additive. And since $(a, b] - \bigcup\limits_{j=1}^m I_j \in \mathcal{A}$ for all $m$, we thus know that:

            {\centering $\mu_0((a, b]) = \mu_0(\bigcup\limits_{j=1}^m I_j) + \mu_0((a, b] - \bigcup\limits_{j=1}^m I_j) \geq \mu_0(\bigcup\limits_{j=1}^m I_j) = \sum\limits_{j=1}^m \mu_0(I_j)$ \retTwo\par}

            Taking the limit as $m \rightarrow \infty$, we get that: $\sum\limits_{j=1}^\infty \mu_0(I_j) \leq \mu_0(\bigcup\limits_{j\in \mathbb{N}} I_j)$.\retTwo
         \end{myIndent}
      \end{enumerate}
   \end{myIndent}
\end{myIndent}
...
\begin{myIndent}\hTwo
   \begin{myIndent}\hFour
      ...
      \begin{tabular}{l}
         ...
         \phantom{aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa} {\teachComment(again since $F$ is monotone increasing)}\\ [6pt]
      ...
      Since $\varepsilon$ is arbitrary, we've now shown the reverse inequality when $a$ and $b$ are finite. To extend this result to when $a = -\infty$ or $b = \infty$, note that the invervals $(a_j, b_j + \delta_j)$ cover $[-M  + \delta, b]$ or $[a + \delta, M]$.\retTwo
   ...
   \blab{Theorem:} 
   \begin{enumerate}
      \item If $F$ is a monotone increasing and right-continuous, there is a unique Borel\\ measure $\mu_F$ on $\mathcal{B}_\mathbb{R}$ such that $\mu_F((a, b]) = F(b) - F(a)$ for all $a < b$.
      \item If $G$ is another monotone increasing and right-continuous, then $\mu_G = \mu_F$ if and only if $F - G$ is a constant.
      ...
      Then $F$ is monotone increasing and right-continuous with $\mu = \mu_F$.
   \end{enumerate}
\end{myIndent}
