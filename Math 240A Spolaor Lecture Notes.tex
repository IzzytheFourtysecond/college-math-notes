...
\begin{myIndent}\exOne
   ...
   \hTwo 
   Based on that exercise, we can without that overly much difficulty show the\\ following lemma.\retTwo

   \blab{Lemma:} Suppose $(X, \mathcal{M})$ is a measurable space. Given $f: X \longrightarrow \overline{\mathbb{R}}$, the following are equivalent:
   \begin{itemize}
      \item $f$ is measurable.
      \item $f^{-1}((a, \infty]) \in \mathcal{M}$ for all $a \in \mathbb{R}$.
      \item $f^{-1}([-\infty, a)) \in \mathcal{M}$ for all $a \in \mathbb{R}$.\retTwo
   ...
   \begin{myIndent}\hThree
      I'll only show that the second bullet point implies the first. This is because the\\ process of showing that the third bullet point implies the first is mostly identical.\\ Plus, it's trivial that the first bullet point implies the other two.\retTwo

      Note that $\{\infty\} = \bigcap\limits_{n \in \mathbb{N}}(n, \infty]$ and $\{-\infty\} = \left(\bigcup\limits_{n \in \mathbb{N}}(-n, \infty]\right)^\comp$.\retTwo

      It follows that $f^{-1}(\{-\infty\}) = \bigcap\limits_{n \in N}f^{-1}((n, \infty]) \in \mathcal{M}$. And similarly, we also have\\ [-8pt] that $f^{-1}(\{-\infty\}) \in \mathcal{M}$.\retTwo

      Next note that $f^{-1}((a, \infty)) = f^{-1}((a, \infty]) - f^{-1}(\{\infty\}) \in \mathcal{M}$ for all $a \in \mathbb{R}$. Also,\\ $f^{-1}(\mathbb{R}) = \bigcup\limits_{n \in \mathbb{N}}f^{-1}((-n, \infty]) - f^{-1}(\{\infty\}) \in \mathcal{M}$.\retTwo
      
      It follows that for all $E \in \mathcal{B}_{\mathbb{R}}$, we have that $f^{-1}(E) \in \mathcal{M}$. And with that, we can now also easily conclude that $f$ is measurable on $f^{-1}(\mathbb{R})$.\retTwo

      So finally, apply the previous exercise.\retTwo
   \end{myIndent}
\end{myIndent}
...
\begin{myIndent}\hTwo
   ...
   \begin{myIndent}\hThree
      ...
      \begin{myIndent}\hFour
         I'll show where the first of those equalities come from. Showing the other would be basically the same.\retTwo
   ...
   \begin{myIndent}\hThree
      To show this, apply the corollary on page 41 to consider the components of $f$ and of each $f_j$. Then use the previous proposition.\newpage
   \end{myIndent}
\end{myIndent}
...
Let $C$ be the cantor set. Given $x \in C$, we know that $x = \sum\limits_{j = 0}^\infty a_j 3^{-j}$ where $a_j = 0$ or $2$ for\\ [-12pt] all $j$. Thus define $f(x) = \sum\limits_{j=0}^\infty b_j 2^{-j}$ where $b_j = \frac{a_j}{2}$.\retTwo

Next, note from the textbook that $f$ is a monotone increasing function on $C$. Also, if $a< b$ with $a$ and $b$ being endpoints of an interval removed from $[0, 1]$ to obtain $C$, then\\ $f(a) = f(b)$. So, we can extend $f$ to all of $[0, 1]$ by making $f$ constant on all the intervals missing from $C$. Then $f$ is called the \udefine{Cantor function}, and it is a continuous, monotone-increasing function from $[0, 1]$ to $[0, 1]$\retTwo

\blab{Exercise 2.9b:} Let $f: [0, 1] \longrightarrow [0, 1]$ be the Cantor function, and let $g(x) = f(x) + x$.


\begin{itemize}
   \item[(b)] If $C$ is the Cantor set, then $m(g(C)) = 1$.
\end{itemize}

\newpage

\blab{Exercise 2.10:} Prove the following are true if and only if $(X, \mathcal{M}, \mu)$ is complete:
\begin{enumerate}
   \item[(a)] If $f$ is measurable and $f = g$ $\mu$-a.e., then $g$ is measurable.
   
   \begin{myIndent}\exTwoP
      Proof:\\
      Consider the set $g^{-1}((a, \infty])$.
   \end{myIndent}

   \item[(b)] If $f_n$ is measurable for all $n \in \mathbb{N}$ and $\lim\limits_{n \rightarrow \infty}f_n(x) = f(x)$ $\mu$-a.e., then $f$ is measurable.\retTwo
